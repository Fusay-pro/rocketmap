...
> **Superseded in part (2026-08-05).** Bug 1 below concluded that `createRow()`
> rejects a `rowId` parameter. That conclusion is wrong: `node-appwrite@22`
> declares `rowId: string` as **required**, and `lib/ai/chat-persistence.ts`
> today passes `rowId: ID.unique()` and works. Whatever the original 500 was, it
> was not the presence of the parameter. Kept as a historical record — do not
> follow its `rowId` guidance. See `docs/APPWRITE_TABLESDB_MIGRATION.md`.
